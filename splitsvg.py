...
def extract_paths_from_svg(svg_file):
    ...
    for i, group in enumerate(groups):
        for j, path in enumerate(group.findall('.//svg:path', ns)):
            ...
            # Create a new group and add the path to this group
            ...
            output_file = os.path.join(output_dir, f"group_{i + 1}_path_{j + 1}.svg")
    ...
    if path_count == 0:
        print("No paths found to export.")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Extracts all paths from an SVG file and saves them as separate SVG files.")
